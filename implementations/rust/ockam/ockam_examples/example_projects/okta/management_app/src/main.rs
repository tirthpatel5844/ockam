...
use oktaplugin::{EstablishmentBundle, KeyEstablishment, Messages, OckamMessages, OckamRole};
use rand::prelude::*;
...
fn main() {
    ...
    loop {
        ...
        match msg {
            ...
            Messages::OktaAccessToken { token } => {
                ...
                if credential_key.is_some() {
                    let msg = OktaRequest {
                        ...
                        msg: OckamMessages::ListServicesRequest {
                            limit: 1000,
                            offset: 0,
                        },
                    };
                ...
                let res = vault.secret_generate(SecretAttributes::new(
            ...
            Messages::OktaResponse { msg } => match msg {
                ...
                OckamMessages::ListServicesResponse { services } => {
                    ...
                    let req = OktaRequest {
                        token: access_token.clone(),
                        msg: OckamMessages::GetEstablishmentBundlesRequest { services: bundles },
                    };
                ...
                OckamMessages::GetEstablishmentBundlesResponse { services } => {
                    ...
                    let res =
                        TcpStream::connect(format!("{}:{}", cfg.truck_address, cfg.truck_port));
                    while res.is_err() {
                        sleep(std::time::Duration::from_secs(2));
                        println!(
                            "Waiting for truck at {}:{}",
                            cfg.truck_address, cfg.truck_port
                        );
                    }
                    ...
                    serde_json::to_writer(
                        &truck_stream,
                        &OckamMessages::BeginDeviceEnrollment { nonce: cred_id },
                    )
                    .unwrap();
                    let mut truck_de =
                        serde_json::Deserializer::from_reader(truck_stream.try_clone().unwrap());
