...
impl TcpPortalSendWorker {
    fn prepare_message(&mut self, msg: &impl AsRef<ockam_core::Encoded>) -> Result<Vec<u8>> {
        let msg = PortalMessage::decode(msg.as_ref())?;
    ...
    async fn handle_message(&mut self, ctx: &mut Context, msg: Routed<Any>) -> Result<()> {
        ...
        if recipient == self.internal_address {
            ...
            while let Some(msg) = self.buffer.pop_front() {
                let msg =
                    TransportMessage::v1(onward_route.clone(), self.remote_address.clone(), msg);
                ctx.forward(LocalMessage::new(msg, Vec::new())).await?;
            }
        } else {
            // Send to Tcp stream
