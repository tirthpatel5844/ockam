...
impl<V: XXVault> State<V> {
    ...
    async fn async_prologue(&mut self) -> Result<()> {
        ...
        if let Some(ik) = &self.identity_key {
            ...
            self.identity_public_key =
                Some(self.vault.async_secret_public_key_get(static_secret_handle.clone()).await?);
            self.identity_key = Some(static_secret_handle)
        };

        // 2. Generate an ephemeral key pair for this handshake and set it to e
        let ephemeral_secret_handle = self.vault.async_secret_generate(attributes).await?;
        self.ephemeral_public = Some(self.vault.async_secret_public_key_get(ephemeral_secret_handle.clone()).await?);
        self.ephemeral_secret = Some(ephemeral_secret_handle);
    ...
    async fn async_encrypt_and_mix_hash<B: AsRef<[u8]>>(
        ...
        let ciphertext_and_tag = {
            let key = self.dh_state.key().ok_or(XXError::InvalidState)?;
            self.vault
                .async_aead_aes_gcm_encrypt(key, plaintext.as_ref(), nonce.as_ref(), h).await?
        };
    ...
    async fn async_decrypt_and_mix_hash<B: AsRef<[u8]>>(
        ...
        let plaintext = {
            let key = self.dh_state.key().ok_or(XXError::InvalidState)?;
            self.vault
                .async_aead_aes_gcm_decrypt(key, ciphertext, nonce.as_ref(), h).await?
        };
    ...
    async fn async_split(&mut self) -> Result<(Secret, Secret)> {
        ...
        let mut hkdf_output =
            self.vault
                .async_hkdf_sha256(ck, b"", None, vec![attributes, attributes]).await?;
...
impl<V: XXVault> State<V> {
    ...
    pub(crate) async fn async_encode_message_1<B: AsRef<[u8]>>(&mut self, payload: B) -> Result<Vec<u8>> {
        let ephemeral_public_key = self
            .ephemeral_public
            .as_ref()
            .ok_or(XXError::InvalidState)?
    ...
    /// Decode the second message in the sequence, sent from the responder
    pub(crate) async fn async_decode_message_2<B: AsRef<[u8]>>(&mut self, message: B) -> Result<Vec<u8>> {
        let public_key_size = CURVE25519_PUBLIC_LENGTH;
        let message = message.as_ref();
        if message.len() < 2 * public_key_size + AES_GCM_TAGSIZE {
            return Err(XXError::MessageLenMismatch.into());
        ...
        self.dh_state.async_dh(&ephemeral_secret_handle, &re).await?;
        self.remote_ephemeral_public_key = Some(re);
        let (rs, h) = self.async_decrypt_and_mix_hash(encrypted_rs_and_tag).await?;
        self.h = Some(h);
        let rs = PublicKey::new(rs);
        self.dh_state.async_dh(&ephemeral_secret_handle, &rs).await?;
        self._remote_static_public_key = Some(rs);
        self.nonce = 0;

        let (payload, h) = self.async_decrypt_and_mix_hash(encrypted_payload_and_tag).await?;
        self.h = Some(h);
        self.nonce += 1;
        Ok(payload)
    }
    ...
    /// Encode the final message to be sent
    pub(crate) async fn async_encode_message_3<B: AsRef<[u8]>>(&mut self, payload: B) -> Result<Vec<u8>> {
        let static_secret = self.identity_key.clone().ok_or(XXError::InvalidState)?;
        ...
        let (mut encrypted_s_and_tag, h) = self.async_encrypt_and_mix_hash(static_public.as_ref()).await?;
        self.h = Some(h);
        self.dh_state
            .async_dh(&static_secret, &remote_ephemeral_public_key).await?;
        self.nonce = 0;
...
impl<V: XXVault + Sync> State<V> {
    ...
    /// Decode the first message sent
    pub(crate) async fn async_decode_message_1<B: AsRef<[u8]>>(&mut self, message_1: B) -> Result<Vec<u8>> {
        let public_key_size = CURVE25519_PUBLIC_LENGTH;
        let message_1 = message_1.as_ref();
        if message_1.len() < public_key_size {
            return Err(XXError::MessageLenMismatch.into());
    ...
    /// Encode the second message to be sent
    pub(crate) async fn async_encode_message_2<B: AsRef<[u8]>>(&mut self, payload: B) -> Result<Vec<u8>> {
        let static_secret = self.identity_key.clone().ok_or(XXError::InvalidState)?;
        let static_public = self
            .identity_public_key
            .clone()
        ...
        self.dh_state
            .async_dh(&ephemeral_secret, &remote_ephemeral_public_key).await?;
    ...
    /// Decode the final message received for the handshake
    pub(crate) async fn async_decode_message_3<B: AsRef<[u8]>>(&mut self, message_3: B) -> Result<Vec<u8>> {
        let public_key_size = CURVE25519_PUBLIC_LENGTH;
        let message_3 = message_3.as_ref();
        if message_3.len() < public_key_size + AES_GCM_TAGSIZE {
            return Err(XXError::MessageLenMismatch.into());
        ...
        let (rs, h) = self.async_decrypt_and_mix_hash(&message_3[..public_key_size + AES_GCM_TAGSIZE]).await?;
        self.h = Some(h);
        let rs = PublicKey::new(rs);
        self.dh_state.async_dh(ephemeral_secret, &rs).await?;
        self.nonce = 0;
        let (payload, h) =
            self.async_decrypt_and_mix_hash(&message_3[public_key_size + AES_GCM_TAGSIZE..]).await?;
        self.h = Some(h);
        self.nonce += 1;
        self._remote_static_public_key = Some(rs);
        Ok(payload)
