...
impl<V: XXVault> DhState<V> {
    ...
    /// Perform the diffie-hellman computation
    pub(crate) async fn async_dh(&mut self, secret_handle: &Secret, public_key: &PublicKey) -> Result<()> {
        let ck = self.ck.as_ref().ok_or(XXError::InvalidState)?;
        ...
        let ecdh = self.vault.async_ec_diffie_hellman(secret_handle, public_key).await?;

        let mut hkdf_output =
            self.vault
                .async_hkdf_sha256(ck, b"", Some(&ecdh), vec![attributes_ck, attributes_k]).await?;
